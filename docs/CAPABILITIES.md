...
V3.0 新增手工浏览器代理入口；V3.13 在 48 个插件基础上增加按插件解析和筛选单个参数的能力，并让 URL/form 证据同时提供可读预览和原始报文。浏览器正常访问 HTTP 测试系统时，代理负责转发、捕获、静态资源过滤、接口指纹去重和有界排队；扫描执行仍复用单报文引擎及其请求预算、Host 限流、证据分级和覆盖率。
...
- `Deep`：勾选 46 项，SQL 深度和 XSS 深度包含各自快速探针，因此取消重复的快速项。
- `Custom`：保留当前手工勾选组合。
...
高成本能力通过独立插件表达，例如 `command_injection_oast`、`file_upload_execution`。旧 `file_read_encoded` 仅作为兼容输入，统一映射到 `file_read` 深度规则。文件读取、文件上传和短信参数名采用受控模糊匹配，覆盖数字索引、分隔符和有限业务修饰词。SQL 在 V3.8.2 由 `sqli`（快速）和 `sqli_deep`（深度）承载；深度包含快速以及旧时间、排序、分页、MyBatis 和堆叠能力，旧 SQL ID 映射到深度。
...
| `file_read` | 任意文件读取（统一） | 是 | Normal 基础路径，Deep 追加 URL/双重/混合编码、分号归一化和 file URI；统一双请求确认 |
...
| `reflected_xss` | 反射型 XSS（快速） | 是 | 唯一标记定位反射上下文，覆盖常见 IMG/onerror、details/ontoggle 和标签闭合 |
| `reflected_xss_deep` | 反射型 XSS（深度） | 否 | 包含快速探针，增加无引号属性、反引号函数、脚本上下文和一行式 Payload |
