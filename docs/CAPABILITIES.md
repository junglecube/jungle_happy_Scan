...
捕获层应在不改变浏览器收到的完整响应前提下，只保存配置上限内的证据副本；Cookie、Token、客户端证书和业务响应均属于敏感数据。共享测试环境建议保持证据脱敏，并限制管理页面和代理监听的可访问网段。
...
- GBK/GB2312/GB18030 的 JSP/Servlet 响应在插件匹配前转 UTF-8；长 HTML 响应使用头部/中部/尾部分段归一化，证据围绕命中点截取并标明响应是否被最大字节限制截断。
