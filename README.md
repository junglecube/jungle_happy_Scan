...
- 修复 SQL、XSS、上传、文件读取、短信、XXE、SSRF 和敏感信息的审计问题，并区分证据可信度与排序分数。
- 前台【持久配置 → 响应语义】新增业务成功／失败规则、短信次数／阈值和回连等待设置。
- 替换对应架构的可执行文件后，继续使用原 config 和启动命令；自动备份并迁移至配置版本 33。
- [V3.9.0 配置与升级说明](docs/RELEASE-v3.9.0.md)。
=======
# jungle_happy_Scan V3.8.3
...
- HTTP 签名接口统一使用 JSON：请求和成功响应均为 `{"request":"<base64>"}`。
- 修复 `preSign.py` 与扫描器协议不一致导致的“签名器返回的内容不是合法的 Base64 HTTP 报文”问题。
- 修复 Linux 下 `PreSignJS` 大小写目录、Node.js 命令和相对工作目录适配；不修改现有 `F-XXX.js` 脚本。

## V3.8.2 更新

- 敏感信息插件新增通用 `flag{...}` 标记检测，内容支持任意字符、空内容和换行。
- 未授权插件新增持久化公开接口白名单，可配置 `/login`、`/checkHealth` 等无需登录的路径；命中后在计划阶段跳过检测。
- 持久配置页面重新整理卡片、编辑区和响应式布局，规则编辑更清晰。
- 敏感信息规则现在正确使用配置中的 severity 等级。

## V3.8.1 更新

- SQL 插件整合为 `sqli`（快速）与 `sqli_deep`（深度）；深度包含快速，重复选择自动去重，旧 ID 兼容映射。
- 快速保留引号破坏/恢复、条件错误和类型适配布尔差分；深度增加扩展闭合、ORDER BY、LIMIT/OFFSET、MyBatis 片段及时间/堆叠探测。
- 补齐 PostgreSQL 无注释尾部、括号闭合、精确替换/追加、LIKE/拼接等时间形式；时间候选先 A/B 筛选，阳性才追加 B/A/半时长 C/A，拒绝固定慢响应和单次抖动。
- 支持稳定应用 500 的时间复核；鉴权失败、限流、网关错误、传输超时不作为阳性。旧 Normal 升级保持快速成本，配置版本升为 32。
- [详细发布说明](docs/RELEASE-v3.8.3.md)；[快速/深度使用说明](docs/plugins.md#58-sql-注入快速sqli)。Linux 最小包不包含 Node；只有 JS 签名模式需要服务器另装 Node。

## V3.8.0 更新

- 扫描请求支持可选应用签名适配：HTTP 签名接口或上传 JavaScript 签名脚本；所有业务字段变异在发送前重新签名。
- `jungle_happy_scan`、Lite、异步扫描和爆破接口均支持 `signature` 参数，未传入时保持原有行为。

## V3.6.5 更新

- 优化敏感信息检测：身份证正则预筛 1900–2030 年、合法月份/日期，并继续执行真实日期与 18 位校验码验证。
- 银行卡默认规则增加国内常见 BIN 头、16–19 位长度、非全重复数字和 Luhn 校验，降低误报。
...
- 修复 Full 扫描 API 的 Evidence 裁剪优先级：保留完整响应头；响应体不超过 64 KiB 时完整保留，超过后才围绕关键证据保留上下各 30 行（最多 61 行），并明确区分捕获截断与证据截取。
...
V1.2 将原 V1.1.1 的功能正式合并升级：管理页面默认显示“扫描中心”、“持久配置”和“版本更新”，也可以使用 `/?config=true` 或 `/?version=true` 直接进入对应页面。外部 API 可只调用 `POST /api/v1/jungle_happy_scan`，连接返回时直接得到最终扫描状态和漏洞数组；同步接口支持 `http` 和可选的上游原始 `response` 参照响应，详见下文和 API 文档。
...
V1.3 新增扫描计划器、公平请求预算和逐插件覆盖率报告，能够明确区分“完整执行”“预算内部分执行”和“不适用跳过”。发现结果统一加入可信分、结论类别、去重和同输入点关联；协议层新增 URL 路径、业务 Cookie、可配置 Header、XML 属性/CDATA、GraphQL variables、multipart 文件名及 Base64 JSON 嵌套变异，并支持强制或 Raw HTTP/1.1。共享服务限流、目标白名单强制和管理 CIDR 仍可在前台持久配置；动态签名、响应 Token 提取保留已有配置兼容能力。
