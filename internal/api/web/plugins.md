<<<<<<< HEAD
# jungle_happy_Scan V3.11.0 插件扫描与配置手册

> 本文以 V3.11.0 为准。反射型 XSS 快速/深度检测、Payload 选择和仅替换二进制升级详见 V3.11.0 发布版本。HTTP 签名接口 JSON request 协议、SQL 快速与深度的差异、时间复核及旧配置升级见 2.2、5.8–5.9；其余章节保留对应能力的演进说明。
=======
...
- **Deep**：从 49 个公开插件中选择 47 项，SQL 深度和 XSS 深度已包含各自快速探针，因此不重复勾选快速插件；同时包含其他漏洞族的扩展检测。
- **Custom**：使用当前手工勾选组合；按钮放在最后，手工增减任何预设后自动进入 Custom。

V3.11.0 中，四种模式只是插件选择预设。前端提交最终 `scan_type` 插件 ID 数组；同一个插件不论由 Normal、Deep 还是 Custom 选中，都执行完全相同的请求序列。异步 API 为兼容旧客户端仍接受 `mode`，但它不会改变 Payload 强度。同步总接口继续通过 `scan_type: ["passive"|"normal"|"deep"]` 选择预设，服务端随即展开为插件 ID。
...
两个插件都先用唯一标记定位每个反射上下文，再逐位置测试配置 Payload。快速插件保留少量低成本探针，并覆盖常见 `<img src=x onerror=...>`、`<details open ontoggle=...>` 和标签闭合；深度插件包含快速探针，另外覆盖无引号属性、反引号函数、`Object.bind`、`Symbol.replace`、脚本字符串/代码上下文以及自索引一行式 Payload，例如 `self[0X10f8809.toString\`36\`]\`1\``。深度选择会自动去重快速插件，避免重复请求。

扫描器逐个原始反射位置匹配完整 Payload，编码副本、XML/JSON 包装或其他位置的回显不会误导结果；响应会再次检查 HTML MIME，并记录命中位置是否处于隐藏/不可见容器以及 CSP 是否会阻断当前内联 Payload。报告低危、较确定的原始反射候选，不声称浏览器执行；尚未增加 DOM／存储型 XSS 或真实浏览器执行复核。
...
请求受全局及目标并发／RPS 管理。证据记录实际发送偏移；窗口不足、未形成请求重叠或测试号码不足会展示覆盖不完整。详细规则 JSON 示例见 V3.11.0 发布说明。
