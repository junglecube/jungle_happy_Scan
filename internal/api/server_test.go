...
>>>>>>> 7e660119acdb144ab49f86bcfe0d35e79c6f9929
	...
	cfg["business_rules"] = []map[string]any{{"name": "V39 API persisted", "json_path": "$.result.code", "equals": "OK", "outcome": "success"}}
	cfg["sms"] = map[string]any{"attempts": 10, "threshold": 3, "window_seconds": 40}
	cfg["callback_late_seconds"] = float64(90)
	...
	_ = response.Body.Close()
	if len(saved.Config.BusinessRules) != 1 || saved.Config.BusinessRules[0].Name != "V39 API persisted" || saved.Config.SMS.Attempts != 10 || saved.Config.CallbackLateSeconds != 90 {
		t.Fatal("V3.9 fields were dropped by config API")
	}
}
