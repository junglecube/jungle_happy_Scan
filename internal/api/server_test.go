...
func TestConfigAPIAndEmbeddedPage(t *testing.T) {
	...
	if !bytes.Contains(page, []byte("cfg-rule-payloads")) || !bytes.Contains(page, []byte(`id="cfg-rule-url-keywords"`)) ||
		...
		!bytes.Contains(page, []byte("V3.6.2")) || !bytes.Contains(page, []byte(`id="proxy-view"`)) ||
		...
		bytes.Contains(page, []byte(`data-view="webscan"`)) || !bytes.Contains(page, []byte(`src="/codemirror.js?v=3.6.2"`)) || !bytes.Contains(page, []byte(`src="/webscan.js?v=3.6.2"`)) ||
		...
		t.Fatalf("V3.6.2 UI assets are missing or obsolete controls remain: %s", page)
	}
