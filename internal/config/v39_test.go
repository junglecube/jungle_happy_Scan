...
func TestV39OldConfigMigrationPreservesSettingsAndBackup(t *testing.T) {
	...
	got := store.Get()
	if got.ConfigVersion != 33 || got.Listen != cfg.Listen || !got.RedactEvidence || got.CallbackBaseURL != "" || !reflect.DeepEqual(got.NormalPlugins, cfg.NormalPlugins) || !reflect.DeepEqual(got.PluginRules["file_read"], cfg.PluginRules["file_read"]) {
		t.Fatal("migration overwrote user configuration")
	}
	if got.SMS.Attempts != 30 || got.CallbackLateSeconds != 120 {
		t.Fatal("missing defaults")
	}
	backup, err := os.ReadFile(path + ".pre-v33.bak")
