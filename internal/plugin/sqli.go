...
func scanSQLInjection(ctx *Context, meta model.PluginMeta, profile sqlScanProfile) ([]model.Finding, error) {
	...
	for _, point := range points {
		...
		if ruleID == "sqli" && len(pointErrorPairs) > 0 {
			...
			ctx.Progress(meta.ID, done, total)
			// The gate is a hint only: another closure can still expose an
			// error when this one is filtered or accepted as literal data.
			_ = sqlQuoteGateSignal(ctx, gateResult, patterns, baselineErrors, baselineStability)
		}
...
func normalSQLBooleanPair(pairs []payloadPair, point httpraw.InsertionPoint) []payloadPair {
	...
	selected := preferredSQLPair(pairs, preferred)
	for _, fallback := range []string{"numeric", "and-string"} {
...
func sqlConfiguredOutcome(response model.Response, cfg config.Config) (string, bool) {
	if len(cfg.BusinessRules) > 0 {
		result := diff.EvaluateBusiness(response, cfg, "sqli", "")
		if result.Outcome != diff.Unknown {
			return string(result.Outcome), true
		}
	}
	body := response.Body
